    ...
    <SafeAreaView className='w-full flex-1 pt-24 bg-white'>
      <View className='w-full flex flex-row items-center justify-center'>
        <View className='w-20 h-20 rounded-full bg-gray-200 mr-4'>
          <Image className='w-full h-full rounded-full' source={{ uri: user.profilePicture ? user.profilePicture : "https://picsum.photos/250/250" }} />
        </View>
        <View className='ml-3'>
      ...
      {user.role === "ADMIN" ? <CustomButton
        ...
        handlePress={() => router.navigate('(admin)/create-candidate')}
        containerStyles='mt-4 w-9/12 mx-auto mt-8'
      /> : null}
      {user.role === "ADMIN" ? <CustomButton
        title='View Results'
        handlePress={() => router.navigate('(admin)/view-results')}
        containerStyles='mt-4 w-9/12 mx-auto mt-8'
      /> : null}
    </SafeAreaView>
  );
